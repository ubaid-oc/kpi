...
def get_form_and_submission_tag_names(form: str, submission: str) -> tuple[str, str]:
    ...
    # Encode to bytes: lxml rejects str input when an encoding declaration
    # is present (<?xml version="1.0" encoding="utf-8"?>).
    tree = etree.ElementTree(check_lxml_fromstring(form.encode()))
    root = tree.getroot()
