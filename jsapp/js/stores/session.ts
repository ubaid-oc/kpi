...
class SessionStore {
  ...
  private verifyLogin() {
    ...
    dataInterface.getProfile().then(
      action('verifyLoginSuccess', async (account: AccountResponse | { message: string }) => {
          ...
          if (currentUserName !== '') {
            ...
            try {
              ...
              if (err === 'logout' || err === 'user-changed') {
                ...
                // Upstream 2.026.12i replaced `actions.auth.logout` with
                // `sessionStore.logOut()`; use the new mechanism here.
                this.logOut()
                return
              }
