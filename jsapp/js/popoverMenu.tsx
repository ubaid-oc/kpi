...
export default class PopoverMenu extends React.Component<PopoverMenuProps, PopoverMenuState> {
  ...
  toggle(evt: HTMLElementEvent<HTMLButtonElement>): boolean | void {
    ...
    if (isBlur && evt.relatedTarget && evt.relatedTarget.dataset && evt.relatedTarget.dataset.popoverMenuStopBlur) {
  ...
  render() {
    ...
    return (
      <bem.PopoverMenu m={wrapperMods}>
        ...
        <bem.PopoverMenu__content m={menuMods}>{this.props.children}</bem.PopoverMenu__content>
      </bem.PopoverMenu>
    )
  }
}
