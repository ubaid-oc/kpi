 ...
 * handling tables.
 */
export default class AssetsTable extends React.Component<AssetsTableProps, AssetsTableState> {
  ...
  onChangeFilter(columnId: string, filterValue: string | null = null) {
    if (this.props.filterColumnId === columnId && this.props.filterValue === filterValue) {
  ...
  renderOrderableHeader(columnDef: AssetsTableColumn) {
    ...
    return (
      <bem.AssetsTableRow__column m={columnDef.id} onClick={this.onChangeOrder.bind(this, columnDef.id)}>
        {!hideLabel && <bem.AssetsTableRow__headerLabel>{columnDef.label}</bem.AssetsTableRow__headerLabel>}
        {!hideIcon && icon}
      </bem.AssetsTableRow__column>
    )
  }
    ...
    return (
      <bem.AssetsTable__footer>
        {this.props.totalAssets !== null && (
          <span>{t('##count## items').replace('##count##', String(this.props.totalAssets))}</span>
        )}

        {/* OpenClinica: Pagination intentionally hidden. */}
